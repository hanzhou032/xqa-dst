...
from modeling_bert_dst_common import (BertForDST)
from modeling_xlmr_dst import (RobertaForDST)
from data_processors import PROCESSORS
from utils_dst_domain_adaptation import (convert_examples_to_features)
...
MODEL_CLASSES = {
    ...
    'roberta': (XLMRobertaConfig, RobertaForDST, XLMRobertaTokenizer),
}
...
def evaluate(args, model, tokenizer, processor, prefix="", domain_excluded=None):
    dataset, features = load_and_cache_examples(args, model, tokenizer, processor, evaluate=True, domain_excluded=domain_excluded)
    ...
    collect_inputs = {}
    collect_outputs = {}
    collect_outputs_total_loss = {}
    collect_outputs_slot_loss = {}
    collect_outputs_class_logits = {}
    collect_outputs_start_logits = {}
    collect_outputs_end_logits = {}
    collect_unique_ids = {}
    collect_values = {}
    collect_input_ids_unmasked = {}
    collect_inform = {}
    collect_domain_flag = {}
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # domain_slot_list = [features[i.item()].domain for i in batch[9]]
            # if model.slot_list[count].split('-')[0] in domain_slot_list[0]:
            #     domain_slot_flag = True
            # else:
            #     domain_slot_flag = False

            if model.slot_list[count].split('-')[0] == domain_excluded:
                domain_slot_flag = True
            else:
                domain_slot_flag = False

            # this is used for main filtering only, so it only calculates the first output and pass the remaining
            # outputs with a redundant saved output
            if count != 0 and not domain_slot_flag:
                outputs = last_outputs
            else:
                outputs = model(**inputs)
                last_outputs = outputs

            collect_inputs[model.slot_list[count]] = inputs
            collect_outputs[model.slot_list[count]] = outputs
            collect_outputs_total_loss = outputs[0]
            collect_outputs_slot_loss[model.slot_list[count]] = outputs[1]
            collect_outputs_class_logits[model.slot_list[count]] = outputs[2]
            collect_outputs_start_logits[model.slot_list[count]] = outputs[3]
            collect_outputs_end_logits[model.slot_list[count]] = outputs[4]
            collect_unique_ids = unique_ids
            collect_values[model.slot_list[count]] = values
            collect_input_ids_unmasked[model.slot_list[count]] = input_ids_unmasked
            collect_inform[model.slot_list[count]] = inform
            collect_domain_flag[model.slot_list[count]] = domain_slot_flag

            # Update dialog state for next turn.
            updates = outputs[2].max(1)[1]
            # update only for positive domain
            if domain_slot_flag:
                for i, u in enumerate(updates):
                    if u != 0:
                        diag_state[model.slot_list[count]][i] = u
        count += 1
        if count >= count_max:
            results = eval_metric(model, collect_inputs, collect_outputs_total_loss, collect_outputs_slot_loss, collect_outputs_class_logits, collect_outputs_start_logits, collect_outputs_end_logits, collect_domain_flag, args)
            preds, ds = predict_and_format(model, tokenizer, collect_inputs, collect_outputs_class_logits, collect_outputs_start_logits, collect_outputs_end_logits, collect_unique_ids,
                                           collect_input_ids_unmasked, collect_values, collect_inform, prefix, ds, collect_domain_flag, args)
            ...
            collect_input_ids_unmasked = {}
            collect_inform = {}
            # print(results1-positive-examples)
...
def eval_metric(model, features, total_loss, per_slot_per_example_loss, per_slot_class_logits, per_slot_start_logits,
                per_slot_end_logits, collect_domain_flag, args):
    metric_dict = {}
    ...
    for slot in model.slot_list:
        ...
        class_label_id = features[slot]['class_label_id']
        start_pos = features[slot]['start_pos']
        end_pos = features[slot]['end_pos']

        # for domain filtering only
        if collect_domain_flag[slot]:
            ...
            class_prediction = torch.tensor(0).to(args.device)
            start_prediction = torch.tensor(0).to(args.device)
            end_prediction = torch.tensor(0).to(args.device)
...
def load_and_cache_examples(args, model, tokenizer, processor, evaluate=False, domain_excluded=None):
    ...
    if os.path.exists(cached_file) and not args.overwrite_cache:  # and not output_examples:
        ...
        # domain adaptation, we remove all relevant domain dialogue from the features
        # domain_excluded = domain_excluded
        domain_list = []
        for slot in model.slot_list:
            if slot.split('-')[0] not in domain_list:
                domain_list.append(slot.split('-')[0])
        domain_list.append('none')
        logger.info('domain %s', domain_list)
        copy_domain_list = domain_list.copy()
        copy_domain_excluded = domain_excluded
        # if test or dev, remove all other domains and leave excluded domain unchanged.
        count_dialogues = 0
        last_excluded_id = None
        for feature in features:
            dialogue_id = feature.guid.split('-')[1]
            for dom in feature.domain:
                if args.predict_type == "dev" or args.predict_type == "test":
                    if dom in domain_excluded:
                        current_excluded_id = dialogue_id
                        if current_excluded_id != last_excluded_id:
                            count_dialogues += 1
                        last_excluded_id = current_excluded_id

        count_dialogue_excluded = 0
        count_dialogue_turns_excluded = 0
        count_final_dialogue = 0
        domain_features = []
        current_excluded_id = None
        last_excluded_id = None
        filtered_dialogue_list = []
        # iterate through features and find the dialogues that should be removed
        for feature in features:
            dialogue_id = feature.guid.split('-')[1]
            # if current feature contains the domain that should be excluded, store the dialogue id
            # if domain_excluded in feature.domain:
            for dom in feature.domain:
                if dom in domain_excluded:
                    current_excluded_id = dialogue_id
                    if current_excluded_id != last_excluded_id:
                        count_dialogue_excluded += 1
                    last_excluded_id = current_excluded_id
                    if dialogue_id not in filtered_dialogue_list:
                        filtered_dialogue_list.append(dialogue_id)

        # only collect dialogues that are not from the filtered dialogue list.
        for feature in features:
            dialogue_id = feature.guid.split('-')[1]
            # excluded all features from that dialogue id
            if args.predict_type == "dev" or args.predict_type == "test":
                domain_features.append(feature) # we dont change the test sets
            else:
                if dialogue_id in filtered_dialogue_list:
                    count_dialogue_turns_excluded += 1
                    continue
                else:
                    domain_features.append(feature)
        logger.info("excluded features %s", count_dialogue_turns_excluded)
        logger.info("excluded dialogues %s", count_dialogue_excluded)
        logger.info('domain dialogues %s', count_dialogues)

        features = domain_features.copy()

        temp_dialogue_id = None
        for feature in features:
            dialogue_id = feature.guid.split('-')[1]
            if dialogue_id != temp_dialogue_id:
                temp_dialogue_id = dialogue_id
                count_final_dialogue +=1
        logger.info("final test dialogues %s", count_final_dialogue)
...
def main():
    ...
    parser.add_argument('--domain_experiment', type=str, default='hotel',
                        help="use hotel, train, taxi, attraction, restaurant")
    args = parser.parse_args()
    domain_excluded = args.domain_experiment
    special_tokens_flag = True
    ...
    logger.info("Start running zero-shot experiment on domain: : %s" % domain_excluded)
    ...
    logger.info("the domain excluded for this experiment is %s", domain_excluded)
    ...
    if args.do_train:
        ...
        train_dataset, features = load_and_cache_examples(args, model, tokenizer, processor, evaluate=False, domain_excluded=domain_excluded)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for cItr, checkpoint in enumerate(checkpoints):
            ...
            # Evaluate
            result = evaluate(args, model, tokenizer, processor, prefix=global_step, domain_excluded=domain_excluded)
