...
def convert_examples_to_features(examples, slot_list, class_types, model_type, tokenizer, max_seq_length,
    ...
    logger.info("slot list  %s" % slot_list)
    if model_type == 'bert':
    ...
    def _tokenize_text(text, tokenizer, model_specs, slot_types_flag=False):
        # if text == "taxi-leaveAt":
        #     text = 'car-pick_up_at'
        # elif text == "taxi-destination":
        #     text = 'car-terminus'
        # elif text == "taxi-departure":
        #     text = 'car-leaving_place'
        # elif text == "taxi-arriveBy":
        #     text = 'car-drop_off_by'
    ...
    domain_hotel_list = []
    domain_train_list = []
    domain_attraction_list = []
    domain_restaurant_list = []
    domain_taxi_list = []

    for slot in slot_list:
        if slot.split('-')[0] == 'hotel':
            domain_hotel_list.append(slot)
        elif slot.split('-')[0] == 'train':
            domain_train_list.append(slot)
        elif slot.split('-')[0] == 'attraction':
            domain_attraction_list.append(slot)
        elif slot.split('-')[0] == 'restaurant':
            domain_restaurant_list.append(slot)
        elif slot.split('-')[0] == 'taxi':
            domain_taxi_list.append(slot)
    logger.info("domain hotel %s" % domain_hotel_list)
    logger.info("domain train %s" % domain_train_list)
    logger.info("domain attraction %s" % domain_attraction_list)
    logger.info("domain restaurant %s" % domain_restaurant_list)
    logger.info("domain taxi  %s" % domain_taxi_list)
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for slot in slot_list:
            ...
            # if value_dict[slot] != 'none' or inform_dict[slot] != 'none':
        ...
        # warning_flag = False
        # for slot in slot_list:
        #     if class_label_id_dict[slot] != 0 and value_dict[slot] == 'none':
        #         warning_flag = True
        #     if class_label_id_dict[slot] == 0 and value_dict[slot] != 'none':
        #         warning_flag = True
        # if warning_flag:
        ...
        if not evaluate:
            ...
            if not active_domain:
                random_none_slot_list = []
                # random_none_slot_list.extend(domain_hotel_list)
                # random_none_slot_list.extend(domain_train_list)
                # random_none_slot_list.extend(domain_attraction_list)
                # random_none_slot_list.extend(domain_restaurant_list)
                # random_none_slot_list.extend(domain_taxi_list)
                #
                # random_none_slot_list.append(np.random.choice(domain_hotel_list))
                # random_none_slot_list.append(np.random.choice(domain_train_list))
                # random_none_slot_list.append(np.random.choice(domain_attraction_list))
                # random_none_slot_list.append(np.random.choice(domain_restaurant_list))
                # random_none_slot_list.append(np.random.choice(domain_taxi_list))
                for slot in slot_list:
                    features.append(
                        InputFeatures(
                            guid=example.guid,
