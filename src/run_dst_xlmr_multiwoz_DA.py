...
def evaluate(args, model, tokenizer, processor, translator, prefix="", domain_excluded=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # domain_slot_flag = [features[i.item()].slot for i in batch[9]]
        ...
        if count >= count_max:
            ...
            preds, ds = predict_and_format(model, tokenizer, collect_inputs, collect_outputs_class_logits, collect_outputs_start_logits, collect_outputs_end_logits, collect_unique_ids,
                                           collect_input_ids_unmasked, collect_values, collect_inform, prefix, ds, collect_domain_flag, args, translator)
...
def predict_and_format(model, tokenizer, features, per_slot_class_logits, per_slot_start_logits, per_slot_end_logits,
                       ids, input_ids_unmasked, values, inform, prefix, ds, collect_domain_flag, args, translator, non_english=False):
    prediction_list = []
    dialog_state = ds
    non_english = False
    ...
    for i in range(len(ids)):
        ...
        for slot in model.slot_list:
            ...
            if class_prediction == model.class_types.index('dontcare'):
                ...
                # logger.info('current translation src %s ', dialog_state[slot])
                temp_ds = dialog_state[slot]
                if non_english:
                    try:
                        dialog_state[slot] = translator.translate(str(dialog_state[slot]), src=lang_type, dest='en').text
                        dialog_state[slot] = dialog_state[slot].lower()
                        if '§' in dialog_state[slot]:
                            dialog_state[slot] = dialog_state[slot].split('§')[2]
                    except:
                        dialog_state[slot] = temp_ds
                        if '§' in dialog_state[slot]:
                            dialog_state[slot] = dialog_state[slot].split('§')[2]
                        logger.info('pass translation %s ', dialog_state[slot])
...
def main():
    ...
    parser.add_argument('--lang_type', type=str, default='en',
                        help="en:English, de:German, it:Itlian")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for cItr, checkpoint in enumerate(checkpoints):
            ...
            # Evaluate
            result = evaluate(args, model, tokenizer, processor, translator, prefix=global_step, domain_excluded=domain_excluded)
