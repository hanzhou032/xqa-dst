...
from modeling_xlmr_dst_joint import (RobertaForDST)
from data_processors import PROCESSORS
from utils_dst_domain_filter_joint import (convert_examples_to_features)
...
def evaluate(args, model, tokenizer, processor, translator, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            domain_logits = outputs[5]
            domain_slot_flag = domain_logits.max(1)
    ...
    for i in range(len(ids)):
        ...
        for slot in model.slot_list:
            ...
            if class_prediction == model.class_types.index('dontcare'):
                ...
                if non_english:
                    try:
                        dialog_state[slot] = translator.translate(str(dialog_state[slot]), src='it', dest='en').text
                        dialog_state[slot] = dialog_state[slot].lower()
                    except:
...
def load_and_cache_examples(args, model, tokenizer, processor, evaluate=False):
    ...
    dataset = TensorListDataset(all_input_ids, all_input_mask, all_segment_ids,
                                ...
                                all_refer_ids,
                                all_diag_state,
                                all_class_label_ids, all_example_index, all_domain_label_ids)
