...
def convert_examples_to_features(examples, slot_list, class_types, model_type, tokenizer, max_seq_length, slot_value_dropout=0.0, evaluate=False):
    ...
    def _truncate_length_and_warn(tokens_a, tokens_b, history, tokens_slot, max_seq_length, model_specs, guid):
        ...
        if len(tokens_a) + len(tokens_b) + len(history) + len(tokens_slot) > max_seq_length - model_specs['TOKEN_CORRECTION']:
            logger.info("Truncate Example %s. Total len=%d." % (guid, len(tokens_a) + len(tokens_b) + len(history) + len(tokens_slot)))
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for slot in slot_list:
            negative_example_flag = True
            if negative_example_flag:
                features.append(
                    InputFeatures(
                        guid=example.guid,
