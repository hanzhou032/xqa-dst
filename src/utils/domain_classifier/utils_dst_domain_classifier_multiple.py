...
def convert_examples_to_features(examples, slot_list, class_types, model_type, tokenizer, max_seq_length,
    ...
    def _tokenize_text(text, tokenizer, model_specs):
        tokens_unmasked = []
    ...
    def _tokenize_ds(ds, tokenizer, model_specs, slot_list):
        tokens_unmasked = []
        token_list = []
        token_list.append('<domain>')
        for token in slot_list:
            if ds[token] != 0:
                if model_specs['MODEL_TYPE'] == 'roberta':
                    token = ' ' + token
                token = convert_to_unicode(token)
    ...
    def _truncate_length_and_warn(tokens_a, tokens_b, history, tokens, max_seq_length, model_specs, guid):
        # Modifies `tokens_a` and `tokens_b` in place so that the total
        # length is less than the specified length.
        # Account for [CLS], [SEP], [SEP], [SEP] with "- 4" (BERT)
        # Account for <s>, </s></s>, </s></s>, </s> with "- 6" (RoBERTa)
        if len(tokens_a) + len(tokens_b) + len(history)  > max_seq_length - model_specs[
            'TOKEN_CORRECTION']:
        ...
        _truncate_seq_pair(tokens_a, tokens_b, history, tokens, max_seq_length - model_specs['TOKEN_CORRECTION'])
    ...
    for (example_index, example) in enumerate(examples):
        ...
        tokens_ds = _tokenize_ds(diag_state_dict, tokenizer, model_specs, slot_list)

        input_text_too_long = _truncate_length_and_warn(
            tokens_a, tokens_b, tokens_history, tokens_ds, max_seq_length, model_specs, example.guid)
        ...
        for slot in slot_list:
            ...
            if class_label_id_dict[slot] != 0:
                if slot.split('-')[0] not in active_domain_list:
                    active_domain_list.append(slot.split('-')[0])
        ...
        for domain in domain_list:
            if domain in active_domain_list:
                class_label_id.append(1)
            else:
                class_label_id.append(0)
        if example_index < 1:
            logger.info("*** Example ***")
            ...
            logger.info("class_label_id: %s" % str(class_label_id))
            logger.info("domain: %s" % str(active_domain_list))
            if active_domain_list:
                logger.info("domain_class_label_id: %s" % str(domain_list.index(active_domain_list[0])))
