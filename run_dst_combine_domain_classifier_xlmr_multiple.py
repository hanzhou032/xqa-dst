...
from utils_dst_combine_domain_classifier import (convert_examples_to_features)
...
MODEL_CLASSES = {
    ...
    'RobertaSC': (XLMRobertaConfig, RobertaForDomainClassifier, XLMRobertaTokenizer),
...
def evaluate(args, model, model_domain, tokenizer, processor, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if count == 0:
                inputs_domain = {'input_ids': batch[10],
                          'input_mask': batch[11],
                          'segment_ids': batch[12]}

                outputs_domain = model_domain(**inputs_domain)
                outputs_domain_logits = outputs_domain[2]

                all_class_prediction = []
                domain_predict = []
                for domain in domain_list:
                    class_prob = torch.softmax(outputs_domain_logits[domain], dim=-1)[:, 1]
                    _, class_prediction = outputs_domain_logits[domain].max(1)
                    all_class_prediction.append(class_prediction)
                    if class_prob >= 0.5:
                        domain_predict.append(domain)

            domain_slot = [features[i.item()].slot for i in batch[9]]
            # print('domain_predict %s', domain_predict)
            # print(domain_slot[0])
            if domain_slot[0].split('-')[0] in domain_predict:
...
def load_and_cache_examples(args, model, tokenizer, processor, evaluate=False):
    ...
    all_input_ids_no_slot = torch.tensor([f.input_ids_no_slot for f in features], dtype=torch.long)
    all_input_mask_no_slot = torch.tensor([f.input_mask_no_slot for f in features], dtype=torch.long)
    all_segment_ids_no_slot = torch.tensor([f.segment_ids_no_slot for f in features], dtype=torch.long)
    ...
    dataset = TensorListDataset(all_input_ids, all_input_mask, all_segment_ids,
                                ...
                                all_refer_ids,
                                all_diag_state,
                                all_class_label_ids, all_example_index, all_input_ids_no_slot, all_input_mask_no_slot, all_segment_ids_no_slot)
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default=None, type=str, required=True,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(
                            ALL_MODELS))
    parser.add_argument("--domain_classifier_path", default=None, type=str, required=True,
                        help="Path to the domain classifier.")
    ...
    config_class_domain, model_class_domain, tokenizer_class_domain = MODEL_CLASSES['RobertaSC']
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    config_domain = config_class_domain.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    ...
    config_domain.dst_dropout_rate = args.dropout_rate
    config_domain.dst_heads_dropout_rate = args.heads_dropout
    config_domain.dst_class_loss_ratio = args.class_loss_ratio
    config_domain.dst_slot_list = dst_slot_list
    config_domain.dst_class_types = dst_class_types
    config_domain.dst_class_labels = dst_class_labels

    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
                                                do_lower_case=args.do_lower_case)
    tokenizer_domain = tokenizer_class_domain.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
                                                do_lower_case=args.do_lower_case)
    ...
    tokenizer_domain.add_special_tokens(special_tokens_dict)

    model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                        config=config)
    model_domain = model_class_domain.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                        config=config_domain)
    model.resize_token_embeddings(len(tokenizer))
    model_domain.resize_token_embeddings(len(tokenizer))
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        # model_domain = model_class_domain.from_pretrained('results-multiwoz2-domain-classifier/checkpoint-11238')
        model_domain = model_class_domain.from_pretrained(args.domain_classifier_path)
        model_domain.to(args.device)
        ...
        for cItr, checkpoint in enumerate(checkpoints):
            ...
            # Evaluate
            result = evaluate(args, model, model_domain, tokenizer, processor, prefix=global_step)
