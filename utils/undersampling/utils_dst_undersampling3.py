...
def convert_examples_to_features(examples, slot_list, class_types, model_type, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if not evaluate:
            negative_out_domain_examples = []
            negative_in_domain_examples = []
            slot_examples = []
            for slot in slot_list:
                if slot in active_slot:
                    slot_examples.append(
                        InputFeatures(
                            ...
                            slot=True))
                if slot.split('-')[0] in active_domain and slot not in active_slot:
                    negative_in_domain_examples.append(
                        InputFeatures(
                            guid=example.guid,
                            input_ids=input_ids_dict[slot],
                            input_ids_unmasked=input_ids_unmasked_dict[slot],
                ...
                if slot.split('-')[0] not in active_domain:
                    negative_out_domain_examples.append(
                        InputFeatures(
                            guid=example.guid,
                            input_ids=input_ids_dict[slot],
                            input_ids_unmasked=input_ids_unmasked_dict[slot],
                            ...
                            class_label_id=class_label_id_dict[slot],
                            slot=True))
            positive_size = len(slot_examples)
            pick_negative_size = round(positive_size/1.5)
            for example in slot_examples:
                features.append(example)
            if pick_negative_size != 0:
                if pick_negative_size > len(negative_in_domain_examples):
                    pick_negative_size = len(negative_in_domain_examples)
                pick_element = np.random.choice(len(negative_in_domain_examples), size=pick_negative_size, replace=False)
                for element in pick_element:
                    features.append(negative_in_domain_examples[element])
                if pick_negative_size > len(negative_out_domain_examples):
                    pick_negative_size = len(negative_out_domain_examples)
                pick_element = np.random.choice(len(negative_out_domain_examples), size=pick_negative_size, replace=False)
                for element in pick_element:
                    features.append(negative_out_domain_examples[element])
